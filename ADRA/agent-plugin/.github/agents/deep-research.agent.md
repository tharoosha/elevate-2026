...
- **Jira** — via the `atlassian` MCP (JQL), for similar or previously resolved tickets, weighted toward Done/Resolved status.
...
**Findings** — grouped by source (codebase / other repos / Confluence / Jira), each item as `<path or link> — <one-line relevance note, and for Jira hits, what was actually implemented>`.
